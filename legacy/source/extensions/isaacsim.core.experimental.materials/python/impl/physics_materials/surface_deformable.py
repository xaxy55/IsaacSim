...
class SurfaceDeformableMaterial(PhysicsMaterial):
    ...
    def __init__(
        ...
        *,
        ...
        if existent_paths:
            ...
            if material_types[0] != "surface":
                raise ValueError(f"Invalid material type detected. Expected: 'surface', got: '{material_types[0]}'")
        else:
            paths = nonexistent_paths
            for path in nonexistent_paths:
                deformableUtils.add_surface_deformable_material(stage, path)
                material = self._get_material(stage, path)
    ...
    def set_surface_thicknesses(
        ...
        *,
        ...
        # USD API
        indices = ops_utils.resolve_indices(indices, count=len(self), device="cpu")
        surface_thicknesses = ops_utils.place(surface_thicknesses, device="cpu").numpy().reshape((-1, 1))
        for i, index in enumerate(indices.numpy()):
            self.prims[index].GetAttribute("omniphysics:surfaceThickness").Set(
                surface_thicknesses[0 if surface_thicknesses.shape[0] == 1 else i].item()
            )
    ...
    def get_surface_thicknesses(
        ...
        *,
        ...
        data = np.zeros((indices.shape[0], 1), dtype=np.float32)
        for i, index in enumerate(indices.numpy()):
            data[i][0] = self.prims[index].GetAttribute("omniphysics:surfaceThickness").Get()
        return ops_utils.place(data, device=self._device)
    ...
    def set_surface_stiffnesses(
        ...
        *,
        ...
        """
        assert (
            stretch_stiffnesses is not None or shear_stiffnesses is not None or bend_stiffnesses is not None
        ), "All 'stretch_stiffnesses', 'shear_stiffnesses' and 'bend_stiffnesses' are not defined. Define at least one of them"
        assert self.valid, _MSG_PRIM_NOT_VALID
        # USD API
        indices = ops_utils.resolve_indices(indices, count=len(self), device="cpu")
        if stretch_stiffnesses is not None:
            stretch_stiffnesses = ops_utils.place(stretch_stiffnesses, device="cpu").numpy().reshape((-1, 1))
        if shear_stiffnesses is not None:
            shear_stiffnesses = ops_utils.place(shear_stiffnesses, device="cpu").numpy().reshape((-1, 1))
        if bend_stiffnesses is not None:
            bend_stiffnesses = ops_utils.place(bend_stiffnesses, device="cpu").numpy().reshape((-1, 1))
        for i, index in enumerate(indices.numpy()):
            prim = self.prims[index]
            if stretch_stiffnesses is not None:
                prim.GetAttribute("omniphysics:surfaceStretchStiffness").Set(
                    stretch_stiffnesses[0 if stretch_stiffnesses.shape[0] == 1 else i].item()
                )
            if shear_stiffnesses is not None:
                prim.GetAttribute("omniphysics:surfaceShearStiffness").Set(
                    shear_stiffnesses[0 if shear_stiffnesses.shape[0] == 1 else i].item()
                )
            if bend_stiffnesses is not None:
                prim.GetAttribute("omniphysics:surfaceBendStiffness").Set(
                    bend_stiffnesses[0 if bend_stiffnesses.shape[0] == 1 else i].item()
                )
    ...
    def get_surface_stiffnesses(
        ...
        # USD API
        indices = ops_utils.resolve_indices(indices, count=len(self), device="cpu")
        stretch_stiffnesses = np.zeros((indices.shape[0], 1), dtype=np.float32)
        shear_stiffnesses = np.zeros((indices.shape[0], 1), dtype=np.float32)
        bend_stiffnesses = np.zeros((indices.shape[0], 1), dtype=np.float32)
        for i, index in enumerate(indices.numpy()):
            prim = self.prims[index]
            stretch_stiffnesses[i] = prim.GetAttribute("omniphysics:surfaceStretchStiffness").Get()
            shear_stiffnesses[i] = prim.GetAttribute("omniphysics:surfaceShearStiffness").Get()
            bend_stiffnesses[i] = prim.GetAttribute("omniphysics:surfaceBendStiffness").Get()
        return (
            ops_utils.place(stretch_stiffnesses, device=self._device),
            ops_utils.place(shear_stiffnesses, device=self._device),
            ops_utils.place(bend_stiffnesses, device=self._device),
        )
    ...
    def are_of_type(paths: str | Usd.Prim | list[str | Usd.Prim]) -> wp.array:
        ...
        for item in paths if isinstance(paths, (list, tuple)) else [paths]:
            ...
            if material is not None:
                status = prim_utils.has_api(
                    material.GetPrim(), ["OmniPhysicsDeformableMaterialAPI", "OmniPhysicsSurfaceDeformableMaterialAPI"]
                )
            data.append(status)
