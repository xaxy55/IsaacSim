...
class Extension(omni.ext.IExt):
    """Extension providing PINK multi-task IK example.

    Registers under the "PINK Examples" menu as "Multi-Task" and provides a UI window
    for demonstrating simultaneous weighted tasks: frame tracking, posture
    regularization, and velocity damping.
    """
    ...
    def _on_window(self, visible: bool) -> None:
        if self._window.visible:
            ...
            self._stage_event_sub_opened = carb.eventdispatcher.get_eventdispatcher().observe_event(
                ...
                observer_name="pink_multi_task._on_stage_opened",
            )
            self._stage_event_sub_closed = carb.eventdispatcher.get_eventdispatcher().observe_event(
                ...
                observer_name="pink_multi_task._on_stage_closed",
            )
            self._timeline_event_sub_play = carb.eventdispatcher.get_eventdispatcher().observe_event(
                ...
                observer_name="pink_multi_task._on_timeline_play",
            )
            self._timeline_event_sub_stop = carb.eventdispatcher.get_eventdispatcher().observe_event(
                ...
                observer_name="pink_multi_task._on_timeline_stop",
            )
