...
import warp as wp
from isaacsim.core.experimental.objects import DistantLight as TargetLight
...
class TestDistantLight(omni.kit.test.AsyncTestCase):
    """Test distant light."""
    ...
    @parametrize(backends=["usd"], prim_class=TargetLight, populate_stage_func=populate_stage)
    async def test_angles(self, prim, num_prims, device, backend):
        """Test angles."""
        for indices, expected_count in draw_indices(count=num_prims, step=2):
            cprint(f"  |    |-- indices: {type(indices).__name__}, expected_count: {expected_count}")
            for v0, expected_v0 in draw_sample(shape=(expected_count, 1), dtype=wp.float32):
                prim.set_angles(v0, indices=indices)
                output = prim.get_angles(indices=indices)
