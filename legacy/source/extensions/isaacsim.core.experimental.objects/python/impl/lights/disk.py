...
"""High level class for creating/wrapping USD Disk Light prims that emit light from one side of a circular disk."""
...
class DiskLight(Light):
        ...
        *,
        ...
        if existent_paths:
            ...
            for path in existent_paths:
                ...
                assert prim.IsA(UsdLux.DiskLight), f"Prim at path {path} is not a DiskLight"
                self._lights.append(UsdLux.DiskLight(prim))
        # create lights
        else:
            paths = nonexistent_paths
            for path in nonexistent_paths:
                self._lights.append(UsdLux.DiskLight.Define(stage, path))
    ...
    def are_of_type(paths: str | Usd.Prim | list[str | Usd.Prim]) -> wp.array:
        ...
        return ops_utils.place(
            [
                (stage.GetPrimAtPath(item) if isinstance(item, str) else item).IsA(UsdLux.DiskLight)
